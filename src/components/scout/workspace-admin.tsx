...
function RoleBadge({ role }: { role: UserRole }) {
  ...
  return (
    <span style={{ fontSize: 12, fontFamily: "var(--font-dm-mono)", padding: "2px 7px", borderRadius: 4, ...s }}>
      {role.toLowerCase()}
    </span>
  );
}
          ...
          <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                ...
                {detail.subscription && (
                  <span style={{ fontSize: 12, fontFamily: "var(--font-dm-mono)", padding: "2px 7px", borderRadius: 4, ...STATUS_STYLES[detail.subscription.status] }}>
...
/* ── Coach Management ── */
type AdminView = "overview" | "coaches" | "prompts";
...
function PromptsPanel() {
  ...
  async function handleSave() {
    ...
    try {
      ...
      setPrompts((prev) => prev.map((p) => p.key === selected ? { ...p, content: updated.content, updatedAt: updated.updatedAt } : p));
      setSaveStatus("saved");
    } catch {
      setSaveStatus("error");
    } finally {
...
function FunnelRow({ label, value, total, color = "#1a3a2f" }: { label: string; value: number; total: number; color?: string }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 0" }}>
      <div style={{ width: 160, fontSize: 13, color: "#3d3530", flexShrink: 0 }}>{label}</div>
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "32px 40px", background: "#F7F5F2" }}>
      ...
      <div style={{ display: "inline-flex", gap: 3, background: "rgba(0,0,0,0.05)", padding: 3, borderRadius: 7, marginBottom: 28 }}>
        {([["overview", "Overview"], ["coaches", "Coaches"], ["prompts", "Prompts"]] as [AdminView, string][]).map(([id, label]) => {
      ...
      {adminView === "prompts" && <PromptsPanel />}
      {adminView === "overview" && <>
