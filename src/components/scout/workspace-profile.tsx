          ...
          {fields.map(({ label, value, href }) => (
            <div key={label} className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3">
              ...
              {href && value !== "—" ? (
                ...
                <button onClick={() => setEditing(true)} className="text-sm text-[#C0B8B0] hover:text-[#C4A86A] transition-colors" style={{ background: "none", border: "none", padding: 0, cursor: "pointer", fontFamily: "var(--font-source-sans), system-ui" }}>
                  Add →
                </button>
              ) : (
          ...
          <div className="space-y-2">
            {skillGoals.map((g) => (
              <div key={`${g.skill}-${g.role}`} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", background: "rgba(196,168,106,0.08)", border: "1px solid rgba(196,168,106,0.25)", borderRadius: 8 }}>
                <div>
                  <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#1A1A1A", marginBottom: 2 }}>{g.skill}</p>
                  <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#7A6020" }}>for {g.role}</p>
                </div>
                <button
                  onClick={() => handleGraduate(g.skill)}
                  disabled={graduating === g.skill}
                  style={{ padding: "6px 12px", background: "#1A3A2F", color: "#E8D5A3", border: "none", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 500, cursor: "pointer", opacity: graduating === g.skill ? 0.6 : 1 }}
                >
...
function DreamRoleTab({ dreamList, setDreamList, onSave, hasResume, userSkills, skillGoals, onAddToLearning }: {
  ...
  return (
    <div style={{ maxWidth: isMobile ? "100%" : 560, paddingBottom: 40 }}>
      <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#52493F", marginBottom: 24, lineHeight: 1.7 }}>
            ...
            <div key={role} style={{ background: "#FFFFFF", borderRadius: 10, border: isOpen ? "1.5px solid #1A3A2F" : "1.5px solid rgba(0,0,0,0.08)", boxShadow: isOpen ? "0 4px 20px rgba(26,58,47,0.08)" : "0 1px 4px rgba(0,0,0,0.04)", overflow: "hidden", transition: "border-color 0.15s, box-shadow 0.15s" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", cursor: "pointer" }} onClick={() => toggleExpand(role)}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, fontWeight: 600, color: "#1A1A1A", marginBottom: 2 }}>{role}</p>
                  {loaded ? (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: scoreColor(loaded.fitScore) }}>
                      {scoreLabel(loaded.fitScore)}
                      {loaded._cachedAt ? ` · ${timeAgo(loaded._cachedAt)}` : ""}
                      {roleNeedsRefresh ? " · refresh score" : ""}
                    </p>
                  ) : !hasResume ? (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890" }}>Upload a resume to see your fit score</p>
                  ) : result === "loading" ? (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890" }}>Analyzing…</p>
                  ) : result === "error" ? (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#C4A86A" }}>Analysis unavailable</p>
                  ) : (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890" }}>Click to analyze fit</p>
                  )}
                </div>
              ...
              {isOpen && (
                <div style={{ borderTop: "1px solid rgba(0,0,0,0.06)", padding: "16px 16px 20px" }}>
                  {result === "loading" && (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#A09890", textAlign: "center", padding: "16px 0" }}>Analyzing your resume against this role…</p>
                  )}
                  {result === "error" && (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#C4A86A" }}>Could not run analysis. Make sure your resume is uploaded and try again.</p>
                  )}
                  {!hasResume && (
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#A09890" }}>Upload your resume in the About tab to unlock gap analysis for this role.</p>
                  )}
                  {loaded && (
                    <>
                      <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#52493F", lineHeight: 1.65, marginBottom: 20 }}>{loaded.summary}</p>
                          ...
                          <div style={{ marginBottom: 20 }}>
                            {haveSkills.length > 0 && (
                              <div style={{ marginBottom: 14 }}>
                                <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, fontWeight: 700, color: "#4A8B6A", textTransform: "uppercase", letterSpacing: "1.1px", marginBottom: 8 }}>What you have</p>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                  {haveSkills.map((skill) => (
                                    <span key={skill} style={{ padding: "5px 11px", background: "rgba(74,139,106,0.1)", border: "1px solid rgba(74,139,106,0.2)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#2D6B4A", display: "inline-flex", alignItems: "center", gap: 5 }}>
                            ...
                            {missingSkills.length > 0 && (
                              <div style={{ marginBottom: 14 }}>
                                <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, fontWeight: 700, color: "#A09890", textTransform: "uppercase", letterSpacing: "1.1px", marginBottom: 8 }}>What you&apos;re missing</p>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                  {missingSkills.map((skill) => (
                                    <button key={skill} onClick={() => handleAddToLearning(skill, role)} style={{ padding: "5px 11px", background: "#FFFDF9", border: "1px dashed rgba(0,0,0,0.18)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#52493F", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 5 }}>
                                      <span style={{ color: "#1A3A2F", fontWeight: 700, fontSize: 14, lineHeight: 1 }}>+</span> {skill}
                                    </button>
                                  ))}
                                </div>
                                <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#B0A898", marginTop: 8, fontStyle: "italic" }}>Tap + to add any skill to your Upskilling queue.</p>
                              </div>
                            )}
                            {learningSkills.length > 0 && (
                              <div style={{ marginBottom: 14 }}>
                                <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, fontWeight: 700, color: "#C4A86A", textTransform: "uppercase", letterSpacing: "1.1px", marginBottom: 8 }}>Working on</p>
                                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                                  {learningSkills.map((skill) => (
                                    <span key={skill} style={{ padding: "5px 11px", background: "rgba(196,168,106,0.12)", border: "1px solid rgba(196,168,106,0.35)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#7A6020", display: "inline-flex", alignItems: "center", gap: 5 }}>
                                      <span style={{ fontSize: 12 }}>→</span> {skill}
                                    </span>
                                  ))}
                                </div>
                            ...
                            <div style={{ marginTop: 4, padding: "10px 14px", background: "rgba(0,0,0,0.025)", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
                              <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", margin: 0, lineHeight: 1.5 }}>
                                Skill insights are based on analysis of thousands of active job postings for this role type.
                              </p>
                              <button onClick={() => handleRefresh(role)} style={{ padding: "5px 12px", background: "#FFFFFF", border: "1px solid #E5DDD0", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#52493F", cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 4, whiteSpace: "nowrap", flexShrink: 0 }}>
                                ↻ Refresh
                              </button>
                            </div>
                          </div>
                      ...
                      {/* Next steps */}
                      <div>
                        <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, fontWeight: 700, color: "#4A8B6A", textTransform: "uppercase", letterSpacing: "1.1px", marginBottom: 10 }}>Next steps</p>
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          {loaded.nextSteps.map((step, i) => (
                            <div key={i} style={{ display: "flex", gap: 8 }}>
                              ...
                              <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#52493F", lineHeight: 1.5 }}>{step}</p>
                            </div>
                          ))}
                        </div>
                      </div>
        ...
        <div>
          {!showSearch ? (
            <button
              ...
              style={{ padding: "10px 18px", background: "transparent", color: "#1A3A2F", border: "1px solid rgba(26,58,47,0.2)", borderRadius: 6, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, cursor: "pointer" }}
            >+ Add a role</button>
          ) : (
            <div>
              <input
                ...
                style={{ width: "100%", padding: "12px 12px", borderRadius: 7, border: "1.5px solid #1A3A2F", fontFamily: "var(--font-source-sans), system-ui", fontSize: isMobile ? 16 : 13, color: "#1A1A1A", background: "#FFFFFF", outline: "none", marginBottom: 10, boxSizing: "border-box" }}
              />
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {searchQuery.trim() && !AVAILABLE_ROLES.map(r => r.toLowerCase()).includes(searchQuery.trim().toLowerCase()) && (
                  <button
                    ...
                    style={{ padding: "6px 14px", background: "#1A3A2F", border: "none", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#E8D5A3", cursor: "pointer" }}
                ...
                {filteredRoles.slice(0, 20).map((r) => (
                  <button
                    key={r}
                    onClick={() => addRole(r)}
                    style={{ padding: "6px 14px", background: "#FFFFFF", border: "1px solid rgba(0,0,0,0.1)", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1A1A1A", cursor: "pointer" }}
                  >{r}</button>
                ))}
                <button onClick={() => { setShowSearch(false); setSearchQuery(""); }} style={{ padding: "6px 12px", background: "transparent", border: "none", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#A09890", cursor: "pointer" }}>Cancel</button>
              </div>
            </div>
          )}
        </div>
    ...
    <div style={{ maxWidth: isMobile ? "100%" : 620, paddingBottom: 40 }}>
      ...
      {skillGoals.length > 0 && (
        <div style={{ marginBottom: 32 }}>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, fontWeight: 700, color: "#52493F", textTransform: "uppercase", letterSpacing: "1.1px", marginBottom: 10 }}>From your target roles</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {skillGoals.map((g) => {
                ...
                <div key={`${g.skill}-${g.role}`} style={{ background: "#FFFFFF", borderRadius: 8, padding: "12px 14px", border: "1px solid rgba(196,168,106,0.3)", display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", gap: 12 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                      <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#1A1A1A" }}>{g.skill}</p>
                      <span style={{ padding: "1px 7px", background: "rgba(196,168,106,0.15)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, color: "#7A6020", fontWeight: 600 }}>for {g.role}</span>
                    </div>
                    {matchedCourse && (
                      <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#4A8B6A" }}>
                  ...
                  <button
                    onClick={() => handleGraduate(g.skill)}
                    disabled={graduating === g.skill}
                    style={{ padding: "10px 14px", minHeight: 44, width: isMobile ? "100%" : undefined, background: "#1A3A2F", color: "#E8D5A3", border: "none", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 500, cursor: "pointer", flexShrink: 0, opacity: graduating === g.skill ? 0.6 : 1 }}
                  >
                    {graduating === g.skill ? "Saving…" : "Mark as acquired"}
                  </button>
                </div>
      ...
      <div style={{ background: "#1A3A2F", borderRadius: 10, padding: isMobile ? "16px" : "16px 20px", marginBottom: 24, display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", gap: isMobile ? 16 : 0 }}>
        <div>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, fontWeight: 600, color: "rgba(232,213,163,0.5)", textTransform: "uppercase", letterSpacing: "1px", marginBottom: 4 }}>Your learning progress</p>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 6 }}>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, fontWeight: 700, color: "#52493F", textTransform: "uppercase", letterSpacing: "1.1px" }}>Recommended paths</p>
        </div>
        <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", marginBottom: 16 }}>
        ...
        {UPSKILL_CATEGORIES.map((cat) => {
          ...
          <div key={cat.title} style={{ marginBottom: 20 }}>
            <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#1A1A1A", marginBottom: 4 }}>{cat.title}</p>
            <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#7A7268", marginBottom: 10 }}>{cat.subtitle}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {sorted.map((item) => {
                ...
                return (
                  <div key={item.id} style={{ background: "#FFFFFF", borderRadius: 8, padding: "14px 16px", border: "1px solid rgba(0,0,0,0.06)", display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", gap: 12 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 7, background: item.platformColor, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                      <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 700, color: "#FFFFFF" }}>{item.platformInitial}</span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2, flexWrap: "wrap" }}>
                        <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#1A1A1A" }}>{item.name}</p>
                        {item.scoutPick && <span style={{ padding: "1px 7px", background: "rgba(196,168,106,0.15)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, color: "#7A6020", fontWeight: 600 }}>Kimchi pick</span>}
                        {isGapMatch && <span style={{ padding: "1px 7px", background: "rgba(74,139,106,0.1)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, color: "#2D6B4A", fontWeight: 500 }}>closes gap</span>}
                      </div>
                      ...
                      <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#7A7268", marginBottom: 3 }}>{item.platform} &middot; {item.duration} &middot; {item.credential}</p>
                      <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: statusColor }}>{statusLabel}</p>
                    </div>
                    <button onClick={() => setProgress({ ...progress, [item.id]: prog === "none" ? "inprogress" : prog === "inprogress" ? "completed" : "inprogress" })}
                      style={{ padding: "10px 14px", minHeight: 44, width: isMobile ? "100%" : undefined, background: prog === "completed" ? "rgba(74,139,106,0.1)" : "#1A3A2F", color: prog === "completed" ? "#4A8B6A" : "#E8D5A3", border: "none", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 500, cursor: "pointer", flexShrink: 0 }}>
      ...
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, fontWeight: 700, color: "#52493F", textTransform: "uppercase", letterSpacing: "1.1px" }}>My learning</p>
          {!showAddForm && (
            <button onClick={() => setShowAddForm(true)} style={{ padding: "5px 12px", background: "transparent", border: "1px solid rgba(26,58,47,0.2)", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#1A3A2F", cursor: "pointer" }}>+ Add your own</button>
          )}
        </div>
          ...
          <div style={{ background: "#FFFFFF", borderRadius: 8, padding: "16px", border: "1px solid #E5DDD0", marginBottom: 12 }}>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10, marginBottom: 10 }}>
              <div style={{ gridColumn: "1 / -1" }}>
                <label style={{ display: "block", fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", marginBottom: 4 }}>Course / Certification name *</label>
                <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="e.g. Google Project Management Certificate"
                  style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #E5DDD0", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1A1A1A", background: "#FFFDF9", outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ display: "block", fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", marginBottom: 4 }}>Platform</label>
                <input value={newPlatform} onChange={(e) => setNewPlatform(e.target.value)} placeholder="e.g. Coursera, Udemy, LinkedIn"
                  style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #E5DDD0", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1A1A1A", background: "#FFFDF9", outline: "none", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ display: "block", fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", marginBottom: 4 }}>Duration</label>
                <input value={newDuration} onChange={(e) => setNewDuration(e.target.value)} placeholder="e.g. 6 weeks"
                  style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #E5DDD0", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1A1A1A", background: "#FFFDF9", outline: "none", boxSizing: "border-box" }} />
              </div>
              <div style={{ gridColumn: "1 / -1" }}>
                <label style={{ display: "block", fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", marginBottom: 4 }}>URL (optional)</label>
                <input value={newUrl} onChange={(e) => setNewUrl(e.target.value)} placeholder="https://…"
                  style={{ width: "100%", padding: "8px 12px", borderRadius: 6, border: "1px solid #E5DDD0", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1A1A1A", background: "#FFFDF9", outline: "none", boxSizing: "border-box" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={addCustomItem} disabled={!newName.trim()} style={{ padding: "7px 16px", background: "#1A3A2F", color: "#E8D5A3", border: "none", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 500, cursor: newName.trim() ? "pointer" : "not-allowed", opacity: newName.trim() ? 1 : 0.5 }}>Add</button>
              <button onClick={() => { setShowAddForm(false); setNewName(""); setNewUrl(""); setNewPlatform(""); setNewDuration(""); }} style={{ padding: "7px 14px", background: "transparent", border: "none", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#A09890", cursor: "pointer" }}>Cancel</button>
            </div>
          </div>
        )}
        ...
        {customItems.length === 0 && !showAddForm ? (
          <div style={{ padding: "24px 0", textAlign: "center" }}>
            <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#C0B8B0" }}>No custom items yet. Add courses, certifications, or tools you&apos;re learning on your own.</p>
          </div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              return (
                <div key={item.id} style={{ background: "#FFFFFF", borderRadius: 8, padding: "14px 16px", border: "1px solid rgba(0,0,0,0.06)", display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", gap: 12 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 7, background: "#E8E2D8", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 700, color: "#7A7268" }}>{(item.platform || item.name).charAt(0).toUpperCase()}</span>
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                      {item.url ? (
                        <a href={item.url} target="_blank" rel="noopener noreferrer" style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#1A1A1A", textDecoration: "none" }}>{item.name}</a>
                      ) : (
                        <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#1A1A1A" }}>{item.name}</p>
                      )}
                    </div>
                    <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#7A7268", marginBottom: 3 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: isMobile ? "wrap" : undefined }}>
                    <button onClick={() => updateCustomStatus(item.id)}
                      style={{ padding: "10px 14px", minHeight: 44, flex: isMobile ? 1 : undefined, background: item.status === "completed" ? "rgba(74,139,106,0.1)" : "#1A3A2F", color: item.status === "completed" ? "#4A8B6A" : "#E8D5A3", border: "none", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", justifyContent: "space-between", marginBottom: 20, gap: isMobile ? 14 : 0 }}>
        <div>
          <h2 style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 16, fontWeight: 700, color: "#1A1A1A", margin: 0, letterSpacing: "-0.2px" }}>RESUME</h2>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#6B6258", marginTop: 6 }}>
        ...
        <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5DDD0", overflow: "hidden" }}>
          {resumes.length === 0 ? (
            <div style={{ padding: "40px 20px", textAlign: "center" }}>
              <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, color: "#A09890" }}>No resume uploaded yet.</p>
              <button
                onClick={() => setShowUploadModal(true)}
                disabled={uploading}
              ...
              >
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, marginBottom: 10 }}>
                  ...
                  >
                    ...
                    <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, fontWeight: 600, color: "#1A1A1A", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.name}</span>
                  </button>
                  {renderResumeMenu(r)}
                </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                  <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#6B6258" }}>
      ...
      <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5DDD0", overflow: "visible", position: "relative" }}>
        ...
        <div style={{
          ...
          {["Resume", "Target Job Title", "Last Modified", "Created", ""].map((col) => (
            <span key={col} style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 600, color: "#A09890" }}>{col}</span>
          ))}
        </div>
          ...
          resumes.map((r) => (
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, fontWeight: 600, color: "#1A1A1A" }}>{r.name}</span>
              ...
              {/* Target job title */}
              <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#C0B8B0" }}>
                —
              </span>

              {/* Last modified */}
              <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#6B6258" }}>
                {timeAgo(r.updatedAt)}
              </span>
  ...
  return (
    <div style={{ borderRadius: 10, border: "1px solid #E5DDD0", background: "#FFFDF9", padding: isMobile ? "14px 16px" : "16px 20px", marginBottom: isMobile ? 16 : 28 }}>
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", marginBottom: 10, gap: isMobile ? 10 : 0 }}>
        <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 11, fontWeight: 600, color: "#C4A86A", textTransform: "uppercase" as const, letterSpacing: "1px", margin: 0, display: "flex", alignItems: "center", gap: 4 }}>
          <SparkleIcon /> Kimchi&apos;s read on you
        </p>
        <button
          onClick={onRefresh}
          disabled={loading}
          style={{ display: "flex", alignItems: "center", gap: 4, padding: isMobile ? "10px 14px" : "4px 10px", minHeight: isMobile ? 44 : undefined, background: "transparent", border: "1px solid #E5DDD0", borderRadius: 5, fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890", cursor: loading ? "not-allowed" : "pointer", opacity: loading ? 0.5 : 1 }}
        >
          <span style={{ fontSize: 13 }}>↻</span> {loading ? "Refreshing…" : "Refresh"}
        </button>
      </div>
      {loading ? (
        <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#A09890" }}>Analyzing your profile…</p>
      ) : data ? (
        <>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, color: "#1C3A2F", lineHeight: 1.65, marginBottom: 12 }}>{data.picture}</p>
          <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6, marginBottom: 12 }}>
            {data.strengths.map((s) => (
              <span key={s} style={{ padding: "4px 10px", background: "rgba(28,58,47,0.08)", borderRadius: 100, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1C3A2F" }}>{s}</span>
            ))}
          </div>
          <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 6, marginBottom: 12 }}>
            {data.targetRoles.map((r) => {
              ...
              return (
                <span key={r.role} style={{ padding: "4px 10px", background: bg, borderRadius: 6, fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: c }}>{r.role} · {r.fit}</span>
              );
            })}
          </div>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#A09890", fontStyle: "italic" }}>{data.honestNote}</p>
        </>
      ) : null}
    </div>
  );
...
function CareerPreferencesPanel({ profile, onSave }: {
  ...
  const inputStyle: React.CSSProperties = { width: "100%", padding: isMobile ? "12px 10px" : "8px 10px", fontSize: isMobile ? 16 : 13, borderRadius: 8, border: "1px solid #E5DDD0", background: "#FFFDF9", color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui", outline: "none", boxSizing: "border-box" };
    ...
    <div style={{ background: "#FFFFFF", borderRadius: 12, padding: isMobile ? "18px 16px" : "20px 24px", border: "1px solid rgba(0,0,0,0.07)" }}>
      ...
      {editing ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          {/* Status */}
          <div>
            <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 6 }}>Status</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {PREF_EMPLOYMENT.map(({ value, label }) => (
                <button key={value} onClick={() => setEmpStatus(empStatus === value ? "" : value)}
                  style={{ textAlign: "left", padding: isMobile ? "12px 12px" : "8px 12px", minHeight: isMobile ? 44 : undefined, borderRadius: 8, border: `1px solid ${empStatus === value ? "#1C3A2F" : "#E5DDD0"}`, background: empStatus === value ? "rgba(28,58,47,0.06)" : "#FFFDF9", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui", cursor: "pointer" }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          {/* Timeline */}
          <div>
            <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 6 }}>Timeline</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {PREF_JOB_TIMELINES.map(({ value, label }) => (
                <button key={value} onClick={() => setTimeline(timeline === value ? "" : value)}
                  style={{ textAlign: "left", padding: isMobile ? "12px 12px" : "8px 12px", minHeight: isMobile ? 44 : undefined, borderRadius: 8, border: `1px solid ${timeline === value ? "#1C3A2F" : "#E5DDD0"}`, background: timeline === value ? "rgba(28,58,47,0.06)" : "#FFFDF9", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui", cursor: "pointer" }}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
            {([["Current salary", currentSalary, setCurrentSalary], ["Target salary", targetSalary, setTargetSalary]] as [string, string, (v: string) => void][]).map(([label, val, setter]) => (
              <div key={label}>
                <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 6 }}>{label}</p>
          ...
          {/* Motivation */}
          <div>
            <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 6 }}>Primary motivation</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {PREF_MOTIVATIONS.map((m) => (
                <button key={m} onClick={() => setMotivation(motivation === m ? "" : m)}
                  style={{ padding: "5px 12px", borderRadius: 100, border: `1px solid ${motivation === m ? "#1C3A2F" : "#E5DDD0"}`, background: motivation === m ? "rgba(28,58,47,0.08)" : "#FFFDF9", fontSize: 12, color: motivation === m ? "#1C3A2F" : "#7A7268", fontFamily: "var(--font-source-sans), system-ui", cursor: "pointer" }}>
                  {m}
                </button>
              ))}
            </div>
          ...
          {/* Priorities */}
          <div>
            <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 6 }}>What matters most</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {PREF_PRIORITIES.map((p) => (
                <button key={p} onClick={() => setPriorities(prev => prev.includes(p) ? prev.filter(x => x !== p) : [...prev, p])}
                  style={{ padding: "5px 12px", borderRadius: 100, border: `1px solid ${priorities.includes(p) ? "#1C3A2F" : "#E5DDD0"}`, background: priorities.includes(p) ? "rgba(28,58,47,0.08)" : "#FFFDF9", fontSize: 12, color: priorities.includes(p) ? "#1C3A2F" : "#7A7268", fontFamily: "var(--font-source-sans), system-ui", cursor: "pointer" }}>
                  {p}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 8, paddingTop: 2 }}>
            <button onClick={handleSave} disabled={saving}
              style={{ padding: "7px 16px", fontSize: 12, fontWeight: 600, background: "#1C3A2F", color: "#F2EDE3", border: "none", borderRadius: 8, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.5 : 1, fontFamily: "var(--font-source-sans), system-ui" }}>
              {saving ? "Saving…" : "Save"}
            </button>
            <button onClick={handleCancel}
              style={{ padding: "7px 16px", fontSize: 12, color: "#52493F", background: "transparent", border: "none", cursor: "pointer", fontFamily: "var(--font-source-sans), system-ui" }}>
              Cancel
            </button>
          </div>
        </div>
        ...
        <div style={{ paddingTop: 4 }}>
          <p style={{ fontSize: 13, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 6 }}>
            No preferences set yet.{" "}
            <button onClick={() => setEditing(true)} style={{ background: "none", border: "none", color: "#C4A86A", fontSize: 13, cursor: "pointer", fontFamily: "var(--font-source-sans), system-ui", padding: 0 }}>
              Add them →
            </button>
          </p>
          <p style={{ fontSize: 12, color: "#C0B8B0", fontFamily: "var(--font-source-sans), system-ui" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {profile.employmentStatus && (
            <div>
              <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 500 }}>Status</p>
              <span style={{ display: "inline-block", padding: "6px 12px", borderRadius: 8, background: "#F7F5F2", border: "1px solid rgba(0,0,0,0.08)", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui" }}>{statusLabel || profile.employmentStatus}</span>
            </div>
          )}
          {profile.jobTimeline && (
            <div>
              <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 500 }}>Timeline</p>
              <span style={{ display: "inline-block", padding: "6px 12px", borderRadius: 8, background: "#F7F5F2", border: "1px solid rgba(0,0,0,0.08)", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui" }}>{timelineLabel || profile.jobTimeline}</span>
            </div>
          )}
          {(profile.currentSalary || profile.targetSalary) && (
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
              {profile.currentSalary && (
                <div>
                  <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 500 }}>Current</p>
                  <span style={{ display: "inline-block", padding: "6px 12px", borderRadius: 8, background: "#F7F5F2", border: "1px solid rgba(0,0,0,0.08)", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui" }}>{profile.currentSalary}</span>
                </div>
              )}
              {profile.targetSalary && (
                <div>
                  <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 500 }}>Target</p>
                  <span style={{ display: "inline-block", padding: "6px 12px", borderRadius: 8, background: "#F7F5F2", border: "1px solid rgba(0,0,0,0.08)", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui" }}>{profile.targetSalary}</span>
                </div>
              )}
            </div>
          )}
          {profile.careerMotivation && (
            <div>
              <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 500 }}>Looking for</p>
              <span style={{ display: "inline-block", padding: "6px 12px", borderRadius: 8, background: "#F7F5F2", border: "1px solid rgba(0,0,0,0.08)", fontSize: 13, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui" }}>{profile.careerMotivation}</span>
            </div>
          )}
          {(profile.priorities || []).length > 0 && (
            <div>
              <p style={{ fontSize: 11, color: "#A09890", fontFamily: "var(--font-source-sans), system-ui", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.04em", fontWeight: 500 }}>Priorities</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                {profile.priorities.map((p) => (
                  <span key={p} style={{ padding: "5px 11px", borderRadius: 8, background: "#F7F5F2", border: "1px solid rgba(0,0,0,0.08)", fontSize: 12, color: "#1C3A2F", fontFamily: "var(--font-source-sans), system-ui" }}>{p}</span>
                ))}
              </div>
            </div>
          )}
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden", background: "#F7F5F2", animation: "fadeIn 0.3s ease both" }}>
      <div ref={scrollRef} style={{ padding: scrollPad, overflowY: "auto", flex: 1, WebkitOverflowScrolling: "touch" }}>
        ...
        <div style={{ marginBottom: isMobile ? 20 : 24 }}>
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, fontWeight: 500, color: "#A09890", letterSpacing: "1.1px", textTransform: "uppercase", marginBottom: 8, lineHeight: 1.4 }}>
          ...
          {isMobile && profile?.headline && (
            <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#52493F", marginBottom: 8, lineHeight: 1.4 }}>
              {profile.headline}
            </p>
          )}
              ...
              <div style={{ marginTop: 14 }}>
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 5, maxWidth: isMobile ? "100%" : 280 }}>
                    <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, color: "#A09890" }}>
                      Profile completeness
                      {missing.length > 0 && <span style={{ marginLeft: 6, color: "#C0B8B0" }}>· {missing.length} missing</span>}
                    </span>
                    <span style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 12, fontWeight: 600, color: pct >= 80 ? "#4A8B6A" : "#C4A86A" }}>
                    ...
                    {missing.map((item, i) => (
                      <button
                        ...
                        style={{
                          ...
                          cursor: "pointer", fontFamily: "var(--font-source-sans), system-ui",
                        }}
                      >
        ...
        {readbackNudge && (
          <div style={{ maxWidth: isMobile ? "100%" : 640, marginBottom: 12, padding: "12px 14px", background: "#F0FFF8", border: "1px solid #A8DFC0", borderRadius: 8, display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "stretch" : "center", justifyContent: "space-between", gap: 12 }}>
            <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#1A7A4A", margin: 0, lineHeight: 1.5 }}>
        ...
        <div style={{ display: "flex", gap: 4, borderBottom: "1px solid rgba(0,0,0,0.08)", overflowX: "auto", marginBottom: page === "about" ? 0 : 24, WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}>
          {PAGE_TABS.map(({ id, label }) => (
            <button
              key={id}
              onClick={() => setPage(id)}
              style={{ padding: isMobile ? "10px 14px" : "8px 16px", minHeight: 44, border: "none", borderRadius: "6px 6px 0 0", background: page === id ? "#1A3A2F" : "transparent", color: page === id ? "#E8D5A3" : "#52493F", fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, fontWeight: 500, cursor: "pointer", transition: "all 0.15s", whiteSpace: "nowrap", flexShrink: 0 }}
            >
              {label}
            </button>
          ))}
          ...
          <div style={{ display: "flex", gap: 2, marginBottom: 24, borderBottom: "1px solid rgba(0,0,0,0.05)", overflowX: "auto", background: "#EDE8DF", paddingLeft: 8, WebkitOverflowScrolling: "touch", scrollbarWidth: "none" }}>
            {ABOUT_SECTIONS.map((s) => (
              <button
                ...
                style={{ padding: isMobile ? "8px 12px" : "6px 14px", minHeight: 44, border: "none", background: activeSection === s ? "rgba(28,58,47,0.1)" : "transparent", color: activeSection === s ? "#1C3A2F" : "#7A7268", fontFamily: "var(--font-source-sans), system-ui", fontSize: isMobile ? 12 : 13, fontWeight: activeSection === s ? 600 : 400, cursor: "pointer", whiteSpace: "nowrap", flexShrink: 0, borderBottom: activeSection === s ? "2px solid #1C3A2F" : "2px solid transparent", transition: "all 0.15s" }}
              >
        ...
        {page === "about" && loading && (
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, color: "#A09890" }}>Loading…</p>
        )}
        {page === "about" && !loading && !profile && (
          <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 14, color: "#A09890" }}>Could not load profile. Please refresh.</p>
        )}
        {page === "about" && profile && (
        ...
        {page === "assets" && profile && (
          <>
            {resumeUploadError && (
              <p style={{ fontFamily: "var(--font-source-sans), system-ui", fontSize: 13, color: "#C05050", marginBottom: 12 }}>
                {resumeUploadError}
              </p>
            )}
